...
def mask_sequence(model, df, df2, embeddings_dict):
    matrix = bl.BLOSUM(80)
    allnuminterpro = []
...
def run_masking_pipeline(model, df, df2, df3, embeddings_dict):
    ...
    generated_sequence_list, sequence_similarity = mask_sequence(model, df, df3, embeddings_dict)
    
    selected_proteins = df[df.index.isin(df2['Index'])]
    
    generated_sequence_list, sequence_similarity = mask_sequence(model, selected_proteins, df3, embeddings_dict)    
